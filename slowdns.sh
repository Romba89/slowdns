...
initslow() {
    [[ $(ss -lu | grep -wc '5300') != '0' ]] && {
        ...
        if [[ "$op" == '1' ]]; then
            ...
            sed -i '/slowdns/d' /etc/SSHPlus/dns/autodns >/dev/null 2>&1
...
echo -e "${RED}[${CYAN}5${RED}] ${YELLOW}SLOWDNS SHADOWSOCKS${SCOLOR}"
        ...
        echo "screen -dmS slowdns $DIR/dnstt-server -udp :5300 -privkey-file $DIR/server.key ${ns} 0.0.0.0:${ptdns}" >/etc/SSHPlus/dns/autodns
